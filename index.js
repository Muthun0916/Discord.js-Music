const { entersState,joinVoiceChannel, createAudioPlayer, createAudioResource, StreamType, VoiceConnectionStatus, AudioPlayerStatus } = require('@discordjs/voice');
...
exports.play = async (options = {}) => {
    ...
    const { interaction, channel, song , show} = options;
...
=======
        if(show==true)await interaction.channel.send(`これ再生するよ ${song} `);
>>>>>>> 68e4891a11150a58f03d569922e2fc1d460cbdcf
...
async function playSong(data, interaction) {
    ...
    let resource = await createAudioResource(ytdl(data.queue[0].url, { filter: 'audioonly' ,highWaterMark: 32 * 1024 * 1024,}), {
